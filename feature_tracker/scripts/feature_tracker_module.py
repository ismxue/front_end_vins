...
class PLFeatureTracker:
    ...
    def readImage(self, new_img):
        ...
        start_time = time()
        self.forwframe_['keyPoint'], self.forwframe_['pointdescriptor'], self.forwframe_['vecline'], self.forwframe_[
            'linedescriptor'], self.forwframe_['valid_points'] = self.extractor.extract(self.new_frame)
        ...
        ########################## 查看 message 信息##############################
        # print("-"*15)
        # for key,value in self.forwframe_.items():
        #     print("-"*5)
        #     value_type=type(value)
        #     print(f'Type of value for key "{key}": {value_type} ')
        #     if key =='PointID' or key == 'lineID':
        #         print(key, type(value[0]))
        #     else:
        #         print("value.ndim: ", value.ndim, "| value type:", value.dtype,
        ...
        if self.curframe_['keyPoint'].shape[1] > 0:
            ...
            for k in range(point_matches.shape[1]):
                ...
                self.forwframe_['PointID'][int(point_matches[0, k])] = self.curframe_[
                    'PointID'][int(point_matches[1, k])]
            ...
            for i in range(num_points):
                if self.forwframe_['PointID'][i] == -1:
                    ...
                    pointdescr_new = np.append(pointdescr_new, self.forwframe_[
                                               'pointdescriptor'][:, i:i+1], axis=1)
                    ...
                    pointdescr_tracked = np.append(pointdescr_tracked, self.forwframe_[
                                                   'pointdescriptor'][:, i:i+1], axis=1)
            ...
            if diff_n > 0:
                if vecpoint_new.shape[1] >= diff_n:
                    for k in range(diff_n):
                        ...
                        pointdescr_tracked = np.append(
                            pointdescr_tracked, pointdescr_new[:, k:k+1], axis=1)
            ...
            if self.curframe_['vecline'].shape[0] > 0:
                ...
                for k in range(index_lines1.shape[0]):
                    self.forwframe_['lineID'][index_lines1[k]] = self.curframe_[
                        'lineID'][index_lines2[k]]

                ################### 将跟踪的线与没跟踪的线进行区分 #####################
                vecline_new = np.zeros((0, 2, 2))
                vecline_tracked = np.zeros((0, 2, 2))
                validpoints_new = np.zeros((0, self.num_samples)).astype(int)
                validpoints_tracked = np.zeros(
                    (0, self.num_samples)).astype(int)
                lineID_new = []
                lineID_tracked = []
                linedescr_new = torch.zeros(
                    (128, 0, self.num_samples)).to(self.device)
                linedescr_tracked = torch.zeros(
                    (128, 0, self.num_samples)).to(self.device)
                ...
                for i in range(num_lines):
                    if self.forwframe_['lineID'][i] == -1:  # -1表示当前ID对应的line没有track到
                        # 没有跟踪到的线则编号为新的
                        self.forwframe_['lineID'][i] = self.all_linefeature_cnt
                        self.all_linefeature_cnt = self.all_linefeature_cnt+1
                        vecline_new = np.append(vecline_new, self.forwframe_[
                                                'vecline'][i:i+1, ...], axis=0)  # 取出没有跟踪到的线信息并放入下一帧
                        lineID_new.append(self.forwframe_['lineID'][i])
                        linedescr_new = torch.cat((linedescr_new, self.forwframe_[
                                                  'linedescriptor'][:, i:i+1, :]), dim=1)
                        # linedescr_new = np.append(linedescr_new, self.forwframe_['linedescriptor'][:,i:i+1,:], axis=1)

                        validpoints_new = np.append(validpoints_new, self.forwframe_[
                                                    'valid_points'][i:i+1, :], axis=0)
                    else:
                        # 当前line已被track
                        lineID_tracked.append(self.forwframe_['lineID'][i])
                        vecline_tracked = np.append(vecline_tracked, self.forwframe_[
                                                    'vecline'][i:i+1, ...], axis=0)
                        linedescr_tracked = torch.cat((linedescr_tracked, self.forwframe_[
                                                      'linedescriptor'][:, i:i+1, :]), dim=1)
                        # linedescr_tracked = np.append(linedescr_tracked, self.forwframe_['linedescriptor'][:,i:i+1,:], axis=1)

                        validpoints_tracked = np.append(validpoints_tracked, self.forwframe_[
                                                        'valid_points'][i:i+1, :], axis=0)
                ...
                if diff_n > 0:
                    if vecline_new.shape[0] >= diff_n:
                        for k in range(diff_n):
                            vecline_tracked = np.append(
                                vecline_tracked, vecline_new[k:k+1, :], axis=0)
                            lineID_tracked.append(lineID_new[k])
                            linedescr_tracked = torch.cat(
                                (linedescr_tracked, linedescr_new[:, k:k+1, :]), dim=1)
                            # linedescr_tracked = np.append(linedescr_tracked, linedescr_new[:,k:k+1,:], axis=1)

                            validpoints_tracked = np.append(
                                validpoints_tracked, validpoints_new[k:k+1, :], axis=0)
                    else:

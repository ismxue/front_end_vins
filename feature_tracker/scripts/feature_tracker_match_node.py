...
from feature_tracker_match import PLFeatureTrackerMatch
...
# def img_callback(extracted_msg, params_dict):
#     feature_tracker_match = params_dict["feature_tracker_match"]
...
#         # scale = 2
#         feature_tracker_match.readImage(extracted_msg)
...
#             feature_points.header = extracted_msg.image.header
#             feature_points.header.frame_id = "world"

#             cur_un_pts, cur_pts, ids, cur_un_img = feature_tracker_match.undistortedPoints()
...
#             feature_lines.header = extracted_msg.image.header
#             feature_lines.header.frame_id = "world"

#             cur_un_vecline, cur_vecline, ids, cur_un_img = feature_tracker_match.undistortedLineEndPoints()


#             # print("len(ids):" ,len(ids), "  j_max:", cur_un_pts.shape[1])
...
#             ptr_toImageMsg.header = extracted_msg.image.header
...
#             lineptr_toImageMsg.header = extracted_msg.image.header
...
#                 pt1 = (int(round(cur_vecline[j, 0, 1])), int(
#                     round(cur_vecline[j, 0, 0])))
#                 pt2 = (int(round(cur_vecline[j, 1, 1])), int(
#                     round(cur_vecline[j, 1, 0])))
#                 # cv2.circle(ptr_image, pt2, 2, (0, 255, 0), thickness=2)
...
if __name__ == '__main__':
    rospy.init_node('feature_tracker_match', anonymous=False)
    ...
    feature_tracker_match = PLFeatureTrackerMatch(my_pointmatch_model, my_linematch_model, CameraIntrinsicParam,
    ...
    sub_img = rospy.Subscriber("feature_extract_forw",ExtractedFeature, img_callback,
                               {"feature_tracker_match": feature_tracker_match,

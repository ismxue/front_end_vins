...
class PLFeatureTrackerMatch:
    def __init__(self,  pointmatch_model, linematch_model, camera_model, num_samples=8, min_point_cnt=150, min_line_cnt=50):
    ...
    def readImage(self, extracted_msg):
        
        start_time=time()
        self.forwframe_['PointID']=list(extracted_msg.PointID)     
        self.forwframe_['keyPoint']=np.asarray(extracted_msg.keyPoint).reshape(extracted_msg.keyPoint_shape)
        self.forwframe_['vecline']=np.asarray(extracted_msg.vecline).reshape(extracted_msg.vecline_shape)
        self.forwframe_['lineID']=list(extracted_msg.lineID)
        device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.forwframe_['linedescriptor']=torch.from_numpy(np.asarray(extracted_msg.linedescriptor).reshape(extracted_msg.linedescriptor_shape)).to(device)
        self.forwframe_['pointdescriptor']=np.asarray(extracted_msg.pointdescriptor).reshape(extracted_msg.pointdescriptor_shape)
        self.forwframe_['valid_points']=np.asarray(extracted_msg.valid_points).reshape(extracted_msg.valid_points_shape)
        bridge = CvBridge()
        self.forwframe_['image']=bridge.imgmsg_to_cv2(extracted_msg.image,"mono8")

        end_time=time()
        print("### deserialize_msg_time: ",end_time-start_time)
